...
- ✅ 已完成 [Stage 3](./stage-3-junior-builder.md)（技術路線）或 [Stage 4](./stage-4-workflow-specialist.md)（自動化路線）
- ✅ 熟練使用 LangChain 和自定義 Tool
- ✅ 有用 Python 部署 API 服務的經驗
- ✅ 理解 Docker 基礎
- ✅ 建議：有 1 個上線的全棧專案經驗
...
```
場景選擇（擇一）：
- 智能電商客服總監：能自己查庫存、分析客戶意圖、自動回信
- AI 合約審查官：能讀取合約文件、標記風險條款、生成審查報告
- AI 供應鏈調度總監：能監控庫存、預測需求、自動生成採購單

要求：
- 包含 RAG 知識庫
- 至少 3 個 Agent 協作
- LangGraph 實現工作流
- Docker 部署 + LangFuse 監控
- 處理 50 條測試案例，記錄準確率
```
...
- [ ] 系統必須包含：RAG 知識庫 + 至少 3 個協作 Agent + 有狀態工作流
- [ ] 部署為可公開訪問的服務（或可演示的內部系統）
- [ ] 通過 50 條測試案例，記錄各項指標（準確率、回應時間、Token 成本）
